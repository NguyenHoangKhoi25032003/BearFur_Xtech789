document.addEventListener("DOMContentLoaded", () => {
    ...
    const productContainer = document.getElementById('product-list-decor');
    const prevButton = document.querySelector('.decor-nav-btn .bi-arrow-bar-left')?.parentElement;
    const nextButton = document.querySelector('.decor-nav-btn .bi-arrow-bar-right')?.parentElement;
    ...
    function displayProducts() {
        ...
        productContainer.classList.add('d-flex', 'flex-nowrap', 'overflow-hidden', 'gap-3');
        ...
        productData.forEach(product => {
            ...
            col.innerHTML = `
                <div class="prod-card h-100 position-relative" style="width: 100%;">
                    ${discountPercentage ? `<div class="prod-discount">${discountPercentage}</div>` : ''}
                    <img src="${product.image}" class="prod-img-top" alt="${product.name}" 
                         data-hover="${product.hoverImage}" data-original="${product.image}" style="width: 100%; height: auto; object-fit: cover;">
